...
2. Signed REST fetch orchestration across the entire account and history lookback, complete account-wide trade-ID watermark and persisted cursor, trustworthy genesis or verified pre-anchor baselines, and atomic cursor + all-order settlement/position reconciliation. The newly added single-order atomic transaction is only one building block. Missing, truncated, foreign or conflicting evidence must remain SAFE_HOLD.
