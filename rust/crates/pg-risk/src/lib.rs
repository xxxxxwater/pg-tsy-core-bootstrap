...
pub fn evaluate_signal(signal: &Signal, now_ns: u64, limits: &RiskLimits) -> RiskDecision {
    if signal.is_expired(now_ns) {
        return RiskDecision::Reject {
            code: "SIGNAL_EXPIRED".into(),
            reason: "signal TTL elapsed".into(),
        };
    }
    if !(0.0..=1.0).contains(&signal.confidence) || signal.confidence < limits.min_confidence {
        return RiskDecision::Reject {
            code: "LOW_CONFIDENCE".into(),
            reason: "confidence below limit".into(),
        };
    }
    if signal.score.abs() > limits.max_abs_signal_score {
        return RiskDecision::Reject {
            code: "INVALID_SCORE".into(),
            reason: "score outside configured bound".into(),
        };
    }
    RiskDecision::Allow
}
...
mod tests {
    ...
    fn expired_signal_fails_closed() {
        ...
        assert!(matches!(
            evaluate_signal(&s, 10, &RiskLimits::default()),
            RiskDecision::Reject { .. }
        ));
    }
}
