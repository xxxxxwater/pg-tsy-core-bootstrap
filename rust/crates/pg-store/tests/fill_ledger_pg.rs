...
use pg_store::{PostgresStore, fill_ledger::{ExecutionFill, FillInsertOutcome, FillLedgerError}};
...
async fn fill_is_atomic_idempotent_owned_and_fenced() {
    ...
    let lease = store.acquire_lease(&format!("fill-test:{nonce}"), &nonce, 30).await.unwrap();
    ...
    assert_eq!(store.insert_execution_fill(&lease, &fill).await.unwrap(), FillInsertOutcome::Inserted);
    assert_eq!(store.insert_execution_fill(&lease, &fill).await.unwrap(), FillInsertOutcome::Duplicate);
    assert_eq!(store.recorded_fill_quantity(&lease, &order.client_order_id).await.unwrap(), Decimal::new(4, 3));

    let mut contradictory = fill.clone();
    contradictory.commission = Decimal::new(2, 3);
    assert!(matches!(store.insert_execution_fill(&lease, &contradictory).await, Err(FillLedgerError::Conflict)));
    let mut second = fill.clone();
    second.trade_id = 6002;
    second.quantity = Decimal::new(6, 3);
    assert_eq!(store.insert_execution_fill(&lease, &second).await.unwrap(), FillInsertOutcome::Inserted);
    assert_eq!(store.recorded_fill_quantity(&lease, &order.client_order_id).await.unwrap(), Decimal::new(10, 3));
    let mut overfill = fill.clone();
    overfill.trade_id = 6003;
    assert!(matches!(store.insert_execution_fill(&lease, &overfill).await, Err(FillLedgerError::Conflict)));
    let mut manual = fill.clone();
    manual.trade_id = 6004;
    manual.client_order_id = format!("pg{}", "f".repeat(32));
    assert!(matches!(store.insert_execution_fill(&lease, &manual).await, Err(FillLedgerError::Unowned)));

    let count: i64 = sqlx::query_scalar("SELECT count(*) FROM execution_fills WHERE client_order_id = $1")
        .bind(&order.client_order_id).fetch_one(store.pool()).await.unwrap();
    assert_eq!(count, 2);
    ...
    assert!(matches!(store.insert_execution_fill(&lease, &after_lease).await, Err(FillLedgerError::FencingLost)));
}
