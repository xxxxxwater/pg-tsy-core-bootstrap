...
use pg_execution::{ExecutionAdapter, ExecutionError, OrderLocator, VenueOrderAck, VenueOrderSnapshot, VenueOrderState, VenuePositionSnapshot};
...
impl ExecutionAdapter for FakeAdapter {
    ...
    async fn find_order_by_client_id(&self, client_id: &str) -> Result<Option<VenueOrderSnapshot>, ExecutionError> {
        Ok(self.remote.lock().unwrap().as_ref().filter(|order| order.client_order_id.as_deref() == Some(client_id)).cloned())
    }
...
impl RuntimeStore for MemoryStore {
    ...
    async fn append_event(&self, _: &str, event_type: &str, _: &Value, _: i64) -> Result<i64, RuntimeStoreError> {
        let mut events = self.event_types.lock().unwrap();
        events.push(event_type.to_owned());
        Ok(events.len() as i64)
    }
    async fn save_order_record(&self, record: &OrderRecord, _: i64) -> Result<(), RuntimeStoreError> {
    ...
    async fn load_position_states(&self, _: Venue) -> Result<Vec<VenuePosition>, RuntimeStoreError> {
        Ok(Vec::new())
    }
    async fn save_position_state(&self, _: &VenuePosition, _: i64) -> Result<(), RuntimeStoreError> { Ok(()) }
    async fn save_reconcile_report(&self, _: Venue, report: &ReconcileReport, _: i64) -> Result<Uuid, RuntimeStoreError> {
        *self.report.lock().unwrap() = Some(report.clone());
        Ok(Uuid::new_v4())
    }
}

fn fixture() -> (Arc<MemoryStore>, Arc<FakeAdapter>, DurableExecution<MemoryStore>) {
    ...
    let store = Arc::new(MemoryStore { order: Mutex::new(local), report: Mutex::new(None), event_types: Mutex::new(Vec::new()), fenced: AtomicBool::new(false) });
    let adapter = Arc::new(FakeAdapter { remote: Mutex::new(Some(remote)) });
    let mut adapters = AdapterRegistry::default();
    adapters.register(Venue::BinancePm, adapter.clone());
    let lease = RuntimeLease { lease_key: "isolated-reconcile-test".into(), holder_id: "test-runner".into(), fencing_token: 7, ttl_seconds: 15 };
...
async fn original_fill_mismatch_survives_runtime_reconcile() {
    ...
    store.order.lock().unwrap().apply_fill(Decimal::from(5)).unwrap();
    {
    ...
    assert_eq!(store.order.lock().unwrap().filled_quantity, Decimal::from(5));
...
async fn venue_order_id_mismatch_is_not_overwritten() {
    let (store, adapter, execution) = fixture();
    adapter.remote.lock().unwrap().as_mut().unwrap().venue_order_id = "999".into();
    let cycle = execution.reconcile_once(Venue::BinancePm).await.unwrap();
    assert!(cycle.report.blocks(Venue::BinancePm, "BTCUSDC"));
    assert_eq!(store.order.lock().unwrap().venue_order_id.as_deref(), Some("42"));
}
...
async fn identity_mismatch_never_adopts_wrong_asset_side_or_size() {
    for case in 0..4 {
        ...
        match case {
            ...
            2 => adapter.remote.lock().unwrap().as_mut().unwrap().requested_quantity = Decimal::from(101),
            _ => store.order.lock().unwrap().side = None,
        }
        let original = store.order.lock().unwrap().clone();
        let cycle = execution.reconcile_once(Venue::BinancePm).await.unwrap();
        assert!(cycle.report.blocks(Venue::BinancePm, "BTCUSDC"), "case {case}");
        assert_eq!(store.order.lock().unwrap().venue_order_id, original.venue_order_id);
...
async fn fencing_loss_prevents_any_mutation_or_venue_calls() {
    ...
    assert!(matches!(result, Err(pg_orchestrator::OrchestratorError::Store(RuntimeStoreError::FencingLost))));
