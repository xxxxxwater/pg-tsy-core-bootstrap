...
impl ExecutionAdapter for LostAckVenue {
    async fn submit(&self, intent: &OrderIntent) -> Result<VenueOrderAck, ExecutionError> {
        ...
        *self.accepted.lock().unwrap() = Some(VenueOrderSnapshot {
        ...
        Err(ExecutionError::Unknown("accepted by exchange, ACK dropped".into()))
    }
    ...
    async fn find_order_by_client_id(
        ...
        Ok(self.accepted.lock().unwrap().as_ref()
...
impl RuntimeStore for MemoryStore {
    ...
    async fn append_event(
        &self, _: &str, event_type: &str, _: &Value, _: i64,
    ...
    async fn save_order_record(
        &self, record: &OrderRecord, _: i64,
    ) -> Result<(), RuntimeStoreError> {
        self.orders.lock().unwrap().insert(record.client_order_id.clone(), record.clone());
        Ok(())
    }
    async fn load_orders_for_venue(
        ...
        Ok(self.orders.lock().unwrap().values()
            .filter(|record| record.venue == venue).cloned().collect())
    }
    ...
    async fn save_position_state(
        &self, _: &VenuePosition, _: i64,
    ) -> Result<(), RuntimeStoreError> {
        Ok(())
    }
    async fn save_reconcile_report(
        &self, _: Venue, _: &ReconcileReport, _: i64,
    ) -> Result<Uuid, RuntimeStoreError> {
        Ok(Uuid::new_v4())
    }
}

fn rig() -> (Arc<MemoryStore>, Arc<LostAckVenue>, DurableExecution<MemoryStore>, OrderIntent) {
    let store = Arc::new(MemoryStore::default());
    let venue = Arc::new(LostAckVenue::default());
    let mut registry = AdapterRegistry::default();
    registry.register(Venue::BinancePm, venue.clone());
    let lease = RuntimeLease {
        lease_key: "isolated".into(), holder_id: "one".into(),
        fencing_token: 7, ttl_seconds: 15,
    };
    let execution = DurableExecution::new(store.clone(), lease, registry);
    let intent = OrderIntent {
        intent_id: Uuid::new_v4(), strategy_id: "only-owned-strategy".into(),
        asset: "BTCUSDC".into(), venue: Venue::BinancePm, side: Side::Buy,
        quantity: Decimal::ONE, limit_price: None,
        effect: ExposureEffect::Increase, source_signal_id: None,
    };
...
async fn exchange_accepts_but_ack_lost_recover_by_identity_without_second_post() {
    ...
    assert!(matches!(result, Err(pg_orchestrator::OrchestratorError::Ambiguous { .. })));
    assert_eq!(venue.posts.load(Ordering::SeqCst), 1);
    assert_eq!(store.orders.lock().unwrap()[&intent.client_order_id()].state, OrderState::Unknown);
    ...
    assert_eq!(store.orders.lock().unwrap()[&intent.client_order_id()].state, OrderState::Open);
    assert_eq!(venue.posts.load(Ordering::SeqCst), 1);
    assert!(execution.recover_ambiguous(Venue::BinancePm).await.unwrap().items.is_empty());
}
...
async fn lost_fencing_blocks_order_before_external_side_effect() {
    ...
    let result = execution.dispatch(&intent).await;
    assert!(matches!(result, Err(pg_orchestrator::OrchestratorError::Store(
