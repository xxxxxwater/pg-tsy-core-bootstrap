...
pub async fn fetch_verified_history<R: SignedTradePageReader>(
    ...
    if externally_verified_anchor == 0 || externally_verified_anchor > i64::MAX as u64 {
        return Err(ExecutionError::Conversion("missing or invalid verified history anchor".into()));
    }
    ...
    for _ in 0..MAX_PAGES {
        ...
        let next = page.next_from_id.ok_or_else(|| {
            ExecutionError::Conversion("UM history page lacks a cursor".into())
        })?;
        if next < cursor || next > i64::MAX as u64 || (!page.short_page && next == cursor) {
            return Err(ExecutionError::Conversion("UM history cursor failed to advance".into()));
        }
        ...
        if terminal {
            ...
            .map_err(|_| ExecutionError::Conversion("UM history ownership or fill verification failed".into()));
        }
        cursor = next;
    }
    Err(ExecutionError::Conversion("UM history exceeded bounded pagination; SAFE_HOLD".into()))
}
...
mod tests {
    ...
    impl SignedTradePageReader for Reader {
        async fn read_page(&self, from_id: u64, limit: u16) -> Result<TradePage, ExecutionError> {
            ...
            if self.fail {
                return Err(ExecutionError::Transport("injected signed REST outage".into()));
            }
            let mut pages = self.pages.lock().unwrap();
            if pages.is_empty() {
                return Err(ExecutionError::Transport("unexpected extra REST read".into()));
            }
            Ok(pages.remove(0))
        }
    }
    ...
    fn fixture() -> (VenueOrderSnapshot, Vec<TradePage>) {
        ...
        let trade = UmTrade {
            symbol: "BTCUSDC".into(), trade_id: 9,
            venue_order_id: "123".into(), side: Side::Buy,
            quantity: Decimal::new(5, 3), price: Decimal::from(80_000),
            quote_quantity: Decimal::from(400),
            commission: Decimal::new(1, 3), commission_asset: "USDC".into(),
            realized_pnl: Decimal::ZERO, trade_time_ms: 1_700_000_000_000,
        };
        (snapshot, vec![TradePage {
            trades: vec![trade], next_from_id: Some(10), short_page: true,
        }])
    }
    ...
    fn owned(snapshot: &VenueOrderSnapshot) -> Vec<OwnedSnapshot<'_>> {
        vec![OwnedSnapshot {
            owner: OwnedOrder {
                ...
                symbol: "BTCUSDC", client_order_id: CLIENT,
                venue_order_id: "123", side: Side::Buy,
            },
            snapshot,
        }]
    }
    ...
    async fn signed_read_yields_evidence_but_never_modifies_orders() {
        ...
        let reader = Reader { pages: Mutex::new(pages), cursors: Mutex::new(Vec::new()), fail: false };
        let result = fetch_verified_history(&reader, 9, &owned(&snapshot)).await.unwrap();
    ...
    async fn missing_anchor_unauthenticated_order_and_failed_rest_are_blocked() {
        let (snapshot, pages) = fixture();
        let reader = Reader { pages: Mutex::new(pages), cursors: Mutex::new(Vec::new()), fail: false };
        assert!(fetch_verified_history(&reader, 0, &owned(&snapshot)).await.is_err());
        assert!(reader.cursors.lock().unwrap().is_empty());
        assert!(fetch_verified_history(&reader, 9, &[]).await.is_err());
        let reader = Reader { pages: Mutex::new(Vec::new()), cursors: Mutex::new(Vec::new()), fail: true };
        assert!(fetch_verified_history(&reader, 9, &owned(&snapshot)).await.is_err());
    }

    #[tokio::test]
    async fn full_page_without_next_progress_never_loops_or_advances() {
        let (snapshot, _) = fixture();
        let reader = Reader { pages: Mutex::new(vec![TradePage {
            trades: vec![], next_from_id: Some(9), short_page: false,
        }]), cursors: Mutex::new(Vec::new()), fail: false };
        assert!(fetch_verified_history(&reader, 9, &owned(&snapshot)).await.is_err());
        assert_eq!(*reader.cursors.lock().unwrap(), vec![9]);
    }
}
