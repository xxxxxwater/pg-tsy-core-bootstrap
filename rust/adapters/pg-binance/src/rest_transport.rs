...
impl BinanceRestClient {
    ...
    pub fn signed_form(
                ...
                .any(|(key, _)| key == "signature" || key == "timestamp")
        {
            return Err(ExecutionError::Conversion(
                "invalid signed parameters".into(),
            ));
        }
    ...
    async fn signed(
        ...
        if method != Method::GET && !self.enable_submission {
            return Err(ExecutionError::Unsupported(
                "Binance mutation capability disabled".into(),
            ));
        }
    ...
    pub async fn submit_order(
        ...
        if prepared.path != NEW_ORDER_PATH || prepared.get("symbol") != Some(SYMBOL) {
            return Err(ExecutionError::Conversion(
                "unexpected prepared order".into(),
            ));
        }
        let params = prepared
            .params
    ...
    pub async fn cancel_order(&self, venue_client_id: &str) -> Result<(), ExecutionError> {
        ...
        if response.get("symbol").and_then(Value::as_str) != Some(SYMBOL)
            ...
            || !matches!(
                ...
                Some("CANCELED" | "FILLED" | "EXPIRED")
            )
        {
            return Err(ExecutionError::Unknown(
                "cancel acknowledgment cannot prove ownership or final status".into(),
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    fn signature_is_deterministic_and_never_contains_secret() {
        ...
        assert!(
            client
                .signed_form(&[("signature".into(), "forged".into())], 1)
                .is_err()
        );
    }
    ...
    fn rest_order_maps_to_durable_identity_and_rejects_manual() {
        ...
        assert_eq!(
            order.client_order_id.unwrap(),
            format!("pg{}", "42".repeat(16))
        );
    ...
    async fn disabled_mutations_fail_before_network() {
        ...
        assert!(matches!(
            client.cancel_order("foreign").await,
