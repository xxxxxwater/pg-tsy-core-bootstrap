...
pub fn ws_stream(symbol: &str, interval_ns: u64) -> Result<String, MarketDataError> {
    ...
    {
        return Err(MarketDataError::Subscription("invalid USD-M market symbol".into()));
    }
...
fn ms_ns(value: u64) -> Result<u64, MarketDataError> {
    value.checked_mul(MS_TO_NS).ok_or_else(|| invalid("timestamp overflow"))
}
...
fn validated_candle(
    ...
    if interval_ms == 0
        ...
        || start_ms.checked_add(interval_ms).and_then(|t| t.checked_sub(1)) != Some(close_ms)
        || recv_ns == 0
    {
    ...
    Ok(Candle {
        ...
        end_ns: ms_ns(start_ms.checked_add(interval_ms).ok_or_else(|| invalid("end overflow"))?)?,
        ts_recv_ns: recv_ns,
        open,
        high,
        low,
...
pub fn decode_ws_closed(
    ...
    if data.get("e").and_then(Value::as_str) != Some("kline")
        ...
        || data.get("E").and_then(Value::as_u64).filter(|ts| *ts > 0).is_none()
        || data.get("st").is_some_and(|kind| kind.as_u64() != Some(1))
    {
        return Err(invalid("wrong event type, symbol or market"));
    }
    let k = data.get("k").ok_or_else(|| invalid("missing kline payload"))?;
    if field(k, "s")? != expected_symbol || field(k, "i")? != interval_name(interval_ns).unwrap_or("") {
        return Err(invalid("nested symbol or interval mismatch"));
    }
    let candle = validated_candle(
        expected_symbol,
        interval_ns,
        k.get("t").and_then(Value::as_u64).ok_or_else(|| invalid("start missing"))?,
        k.get("T").and_then(Value::as_u64).ok_or_else(|| invalid("close missing"))?,
        received_ns,
        field(k, "o")?,
        field(k, "h")?,
        field(k, "l")?,
        field(k, "c")?,
        field(k, "v")?,
        k.get("n").and_then(Value::as_u64).ok_or_else(|| invalid("trade count missing"))?,
    )?;
...
pub fn decode_rest_closed(
    ...
    let rows = rows.as_array().ok_or_else(|| invalid("REST response is not an array"))?;
    if rows.len() > 1500 {
        return Err(invalid("REST page exceeds limit"));
    }
    let mut candles: Vec<Candle> = Vec::with_capacity(rows.len());
    for row in rows {
        let values = row.as_array().filter(|values| values.len() >= 12)
            .ok_or_else(|| invalid("malformed REST tuple"))?;
        let start = values[0].as_u64().ok_or_else(|| invalid("missing REST start"))?;
        let close = values[6].as_u64().ok_or_else(|| invalid("missing REST close"))?;
        let candle = validated_candle(
            symbol,
            interval_ns,
            start,
            ...
            values[4].as_str().ok_or_else(|| invalid("close price missing"))?,
            values[5].as_str().ok_or_else(|| invalid("volume missing"))?,
            values[8].as_u64().ok_or_else(|| invalid("trades missing"))?,
        )?;
    ...
    pub fn accept(&mut self, candle: &Candle) -> Result<bool, MarketDataError> {
        if let Some(previous) = self.previous_start_ns {
            ...
            if previous.checked_add(candle.interval_ns) != Some(candle.start_ns) {
                return Err(MarketDataError::Disconnected("USD-M closed-kline gap or reorder; REST backfill required".into()));
            }
        }
...
mod tests {
    ...
    fn ws(closed: bool) -> Vec<u8> {
        serde_json::to_vec(&json!({"e":"kline", "E":START + 60_000, "s":"BTCUSDC", "st":1,
            "k":{"t":START, "T":START + 59_999, "s":"BTCUSDC", "i":"1m",
                "o":"10", "h":"12", "l":"9", "c":"11", "v":"2", "n":4, "x":closed}})).unwrap()
    }

    #[test]
    fn ws_requires_finalised_exact_usdm_contract() {
        assert!(decode_ws_closed(&ws(false), "BTCUSDC", MINUTE, 10).unwrap().is_none());
        let candle = decode_ws_closed(&ws(true), "BTCUSDC", MINUTE, 10).unwrap().unwrap();
        ...
        assert!(decode_ws_closed(&serde_json::to_vec(&combined).unwrap(), "BTCUSDC", MINUTE, 10).unwrap().is_some());
    }

    #[test]
    fn rest_skips_live_bar_and_detects_missing_bar() {
        let row = |start: u64| json!([start,"10","12","9","11","2",start+59_999,"22",4,"1","11","0"]);
        let data = serde_json::to_vec(&json!([row(START),row(START+60_000)])).unwrap();
        let closed = decode_rest_closed(&data,"BTCUSDC",MINUTE,(START+60_000)*MS_TO_NS).unwrap();
    ...
    fn continuity_accepts_one_bar_once_and_blocks_gaps() {
        let one = decode_ws_closed(&ws(true),"BTCUSDC",MINUTE,10).unwrap().unwrap();
